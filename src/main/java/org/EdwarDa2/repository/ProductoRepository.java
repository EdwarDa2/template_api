...
public class ProductoRepository {
    ...
    public void save(Producto producto) throws SQLException {
        String query = "INSERT INTO productos (nombre,precio,cantidad,id_categoria,id_SubCategoria) VALUES (?,?,?,?,?)";
        try (Connection conn = DatabaseConfig.getDataSource().getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
    ...
    public void update(Producto producto) throws SQLException {
        String query = "UPDATE productos  SET nombre = ?,precio = ?,cantidad = ?,id_categoria = ?,id_subCategoria = ? WHERE id_producto = ?";
        try (Connection conn = DatabaseConfig.getDataSource().getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setString(1, producto.getNombre());
            stmt.setFloat(2, producto.getPrecio());
